...
test('TimePlan 详情页面测试 - 修复无限循环后', async ({ page }) => {
  ...
  await page.screenshot({ path: '/tmp/test16-detail.png' });
  
  // 检查内容是否显示
  const hasLoading = await page.locator('text=加载项目中...').isVisible().catch(() => false);
  const title = await page.locator('text=Orion X').first().isVisible().catch(() => false);
  
  console.log(`Has loading spinner: ${hasLoading}, Has title: ${title}`);
