...
#### 新增功能：按模块分组的迭代规划
- ✅ 按**产品线**一级分组（电子架构、感知算法、规划决策）
- ✅ 按**模块**二级分组（E0架构、视觉感知、行为决策等）
- ✅ MR卡片式展示（类型、负责人、日期、进度）
- ✅ **依赖关系可视化**（显示MR之间的依赖）
- ✅ 折叠/展开产品线面板
- ✅ 点击MR查看详情

#### 视图入口
- 工具栏点击 **"模块规划"** 按钮（NodeIndexOutlined图标）
- 或点击 **"时间迭代"** 按钮查看原时间迭代视图

#### 适用数据
- OrionX v4数据（包含12个MR拆解）
- 需要`module`和`productLine`属性
- 自动识别依赖关系

---

## 🎉 V11.3 完整版 (2026-02-08)
...
- ✅ 移除"状态"标签列，专注核心信息
- ✅ 保留关键列：类型、依赖关系、时长
- ✅ 完整支持 lineplan/gateway/milestone 类型
...
#### 3. OrionX 数据拆解（v4）
- E0架构 → 3个MR（域控、传感器、通信）
- 视觉感知 → 4个MR（预处理、检测、跟踪、语义）
- 行为决策 → 3个MR（状态机、强化学习、评估）
- 路径规划 → 2个MR（全局、局部）
- ✨ 新增12+个内部依赖关系
- ✨ 添加 `module` 和 `productLine` 属性

#### 4. 查看指南
- 📖 查看 `docs/V11.3-USER-GUIDE.md` 了解如何查看更改
- 📖 查看 `docs/VIEW-GUIDE.md` 了解各视图的入口和功能

---

## 🎉 V11.2 类型重命名 (2026-02-08)
